...
@section('content')
    <main>
        ...
        <section>
            <div style="background-image: url('{{ $biographyCms[11]->image_url ? asset('storage/' . $biographyCms[11]->image_url) : asset('frontend/images/sm--banner-01.jpg') }}');"
                class="sm-banner">
                <div class="container">
                    <h1>
                        <span class="sm-banner-color-text">Official Jean <br>Renard</span>
                        Store
                    </h1>
                </div>
            </div>
        ...
        <section class="shop-featured-section shop-featured-section-one">
            <div class="container">
                ...
                <div class="featured-products-container mt_15">
                    @if ($featureProducts->isEmpty())
                        <img class="" style="width: 100%" src="{{ 'frontend/images/coming_soon.jpg' }}"
                            alt="" />
                    @else
                        @foreach ($featureProducts as $featureProduct)
                            <a href="{{ route('product.show', $featureProduct->slug) }}" class="featured-grid-container">
                                @php
                                    $firstImage = $featureProduct->images->first()->image_url;
                                @endphp
                                <img class=""
                                ...
                                <div class="d-flex bottom-wrapper justify-content-between">
                                    <div class="text-container">
                                        <h4>{{ $featureProduct->name ?? 'Feature Product' }}</h4>
                                        @foreach ($featureProduct->ptags as $index => $tag)
                                            <span class="text-secondary">{{ $tag->name }}</span>

                                            @if ($index < $featureProduct->ptags->count() - 1)
                                                ,
                                            @endif
                                        @endforeach
                                        <h4 class="price">€ {{ $featureProduct->price ?? '' }}</h4>
                                    </div>
                                    <div>
                ...
                <div class="d-flex justify-content-center mt-5">
                    {{ $featureProducts->links('vendor.pagination.bootstrap-5') }}
                </div>
            </div>
        </section>
        <!-- featured product section ends -->

        <!-- Books By Jean Renard section start -->
        @if ($categories->isEmpty())
            <section class="shop-featured-section">
                <div class="container">
                    <div class="featured-text-container">
                        <h3 class="shop-h3">Books By Jean Renard</h3>
                    </div>
            ...
            @foreach ($categories as $category)
                @if ($category->products->count() > 0)
                    <section class="shop-featured-section">
                        <div class="container">
                            <div class="featured-text-container">
                                <h3 class="shop-h3">{{ $category->name ?? 'Books' }} By Jean Renard</h3>
                            </div>
                            <div class="featured-products-container mt_15">
                            ...
                            <div class="d-flex justify-content-center mt-5">
                                {{ $category->products->links('vendor.pagination.bootstrap-5') }}
                            </div>
                        </div>
                    </section>
                @endif
